...
export default function HomePage() {
  ...
  return (
    ...
    >
      ...
      {/* Top Navbar */}
      {/* Self-contained dark glass bar: it carries its own contrast, so it stays
          legible over the bright hero photo and the light sections below. */}
      ...
      >
        {/* --- SECTION 1: HERO --- */}
        {/* Deliberately empty apart from the scroll affordance: the hero is the
            clear hospital photo plus the 3D wordmark and ECG line, both of which
            live in the fixed WebGL canvas behind this section. */}
        {!isAiMode && (
          <section className="relative h-[100dvh] w-full snap-start snap-always pointer-events-none">
            <motion.div
              style={{ opacity: heroHintOpacity }}
              className="flex flex-col items-center justify-end h-full w-full px-6"
            >
              {/* Scroll affordance */}
              {/* Dark-on-light glass: the backdrop is a bright daytime photo
                  now, so the previous white-on-white/10 treatment would wash
                  out against the foreground paving. */}
              <motion.div
                animate={{ y: [0, 8, 0] }}
                transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }}
                className="mb-10 flex flex-col items-center gap-1.5 text-slate-700 bg-white/70 backdrop-blur-md px-5 py-2.5 rounded-full border border-white/80 shadow-lg shadow-slate-900/10"
              >
